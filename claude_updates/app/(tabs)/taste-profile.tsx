      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1 justify-center items-center bg-black/40 dark:bg-black/60 p-4">
        <View className="bg-surface dark:bg-darksurface w-full max-w-[340px] rounded-[32px] p-8 shadow-xl border border-black/[0.05] dark:border-darksoftBorder">
          ...
          <View className="flex-row gap-3 mt-8">
            <TouchableOpacity activeOpacity={0.7} onPress={onClose} className="flex-1 py-4 rounded-full border border-black/10 dark:border-white/10 items-center justify-center hover:bg-black/5 dark:hover:bg-white/5">
              <Text className="text-textSec dark:text-darktextSec font-medium text-[15px]">Cancel</Text>
            </TouchableOpacity>
            <TouchableOpacity activeOpacity={0.8} onPress={() => { onSave(tempValue); onClose(); }} className="flex-1 bg-primary py-4 rounded-full items-center justify-center shadow-sm hover:bg-primary-hover">
            ...
            <View>
              <View className="flex-row justify-between items-end mb-6 pl-2">
                ...
                <TouchableOpacity 
                  ...
                  activeOpacity={0.8}
                  className="bg-primary hover:bg-primary-hover px-5 py-3 rounded-full shadow-sm flex-row items-center justify-center"
                >
                ...
                {MOCK_RECIPES.slice(0, 2).map((recipe) => (
                  <View key={recipe.id} className="flex-1 h-[200px] rounded-[32px] overflow-hidden relative shadow-[0_4px_24px_rgba(0,0,0,0.06)] dark:shadow-none dark:border dark:border-darksoftBorder bg-black/5 dark:bg-white/5">
                    {Platform.OS === 'web' ? (
                      ...
                      return src ? (
                        <RNImage source={src} style={{width: '100%', height: '100%', position: 'absolute'}} resizeMode="cover" />
                      ) : (
                        <View style={{width: '100%', height: '100%', position: 'absolute', backgroundColor: '#F1F5F9', alignItems: 'center', justifyContent: 'center'}}>
