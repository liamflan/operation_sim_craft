...
export default function RecipeLibrary() {
  ...
  return (
    <View style={styles.container}>
          ...
          <View style={[styles.searchContainer, isSearchFocused && styles.searchContainerFocused]}>
            <View style={styles.searchIcon}>
              <FontAwesome5
                name="search"
                size={12}
                color={isSearchFocused ? '#9DCD8B' : '#A3B3A9'}
              />
            </View>
            <TextInput
          ...
          >
            {CUISINES.map((cuisine) => (
              <TouchableOpacity
                ...
                style={[
                  styles.filterChip,
                  activeCuisine === cuisine.id && styles.filterChipActive,
                ]}
              >
                <Text
                  style={[
                    styles.filterChipText,
                    activeCuisine === cuisine.id && styles.filterChipTextActive,
                  ]}
                >
                  {cuisine.label}
                </Text>
          ...
          >
            {SORT_OPTIONS.map((sortOption) => (
              <TouchableOpacity
                ...
                style={[styles.sortChip, sortBy === sortOption.id && styles.sortChipActive]}
              >
                <FontAwesome5
                  name={sortOption.icon}
                  size={10}
                  color={sortBy === sortOption.id ? '#24332D' : '#6A766E'}
                  style={{ marginRight: 6 }}
                />
                <Text
                  style={[
                    styles.sortChipText,
                    sortBy === sortOption.id && styles.sortChipTextActive,
                  ]}
                >
                  {sortOption.label}
                </Text>
      ...
      <ImportRecipeModal visible={importModalVisible} onClose={() => setImportModalVisible(false)} />
    </View>
  );
}
...
function RecipeCard({ 
  ...
  const isImported =
    recipe.sourceId?.startsWith('imported_') || recipe.sourceId === 'imported_user';
  const shouldShowImage = Boolean(recipe.imageUrl) && !hasImageError;

  return (
    <TouchableOpacity
      activeOpacity={0.94}
      onPress={() => router.push(`/recipe/${recipe.id}`)}
      style={[styles.card, { width: isDesktop ? '31%' : '100%' }]}
    >
      <View style={styles.imageContainer}>
        <View style={styles.placeholderContainer}>
          ...
          <View style={styles.placeholderContent}>
            ...
            <Text style={styles.placeholderSubtext}>
              {protein >= 35 ? 'High Protein' : cuisine || 'Nutritious'} •{' '}
              {recipe.tags?.[0] || 'Fresh'}
            </Text>
          </View>
        </View>
        ...
        {shouldShowImage && (
          Platform.OS === 'web' ? (
            ...
            return src ? (
              <RNImage source={src} style={styles.image} resizeMode="cover" onError={() => setHasImageError(true)} />
            ) : null;
          })()
        )}
        ...
        <View style={styles.badgeRow}>
          ...
          {status !== 'ready' && (
            <View style={[styles.statusBadge, status === 'draft' && styles.statusDraft]}>
              <Text style={styles.statusBadgeText}>{status === 'draft' ? 'Draft' : 'Review'}</Text>
            </View>
          )}
        </View>
      </View>
