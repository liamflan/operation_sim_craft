...
const ActionButton = ({ icon, label, onPress, disabled, variant = 'secondary' }: { icon: string, label?: string, onPress: () => void, disabled?: boolean, variant?: 'primary' | 'secondary' }) => (
  <TouchableOpacity 
    onPress={onPress}
    disabled={disabled}
    className={`flex-row items-center justify-center w-10 h-10 rounded-full ${variant === 'primary' ? 'bg-textMain dark:bg-darktextMain' : 'bg-transparent'} ${disabled ? 'opacity-50' : 'active:scale-95'}`}
  >
